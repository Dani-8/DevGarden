...
export function registerCafeHandlers(io: any, socket: any) {
    ...
    // Collab Item Like Toggle
    socket.on('cafe_collab_like', async (data: { id: string; increment: boolean }) => {
        try {
            const updated = await toggleCollabLike(data.id, data.increment);
            if (updated) {
                io.emit('cafe_collab_updated', updated);
            }
        } catch (err) {
            console.error('Error handling cafe_collab_like:', err);
        }
    });
}
