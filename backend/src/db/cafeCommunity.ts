...
export async function toggleCollabLike(id: string, increment: boolean): Promise<CollabItem | null> {
    const delta = increment ? 1 : -1;
    const item = inMemoryCollabs.find((c) => c.id === id);
    if (item) {
    ...
    if (isSupabaseConfigured() && item) {
        try {
            const supabase = getSupabase();
            await supabase.from('cafe_collabs').update({ likes: item.likes }).eq('id', id);
        } catch (err) {
            console.warn('Supabase like update error for cafe_collabs:', err);
        }
    }
