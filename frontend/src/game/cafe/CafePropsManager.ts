...
export class CafePropsManager {
    static createProps(
        ...
        // Grand Glowing CODE CAFE /> Neon Plaque (300x88 2x canvas -> 150x44 display)
        const neonSign = scene.add.image(360, 48, 'cafe_neon_sign');
        neonSign.setDisplaySize(150, 44);
        neonSign.setOrigin(0.5, 0.5);
        neonSign.setDepth(20);

        // Wall Poster: "TEA SLEEP CODE REPEAT" (220x140 2x canvas -> 110x70 display)
        const poster = scene.add.image(550, 48, 'cafe_wall_poster');
        poster.setDisplaySize(110, 70);
