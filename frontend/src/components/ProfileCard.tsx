import { X, Github, Award, Star, Flame, Users, Calendar, FolderGit } from 'lucide-react';
...
export default function ProfileCard({ player, onClose }: ProfileCardProps) {
  // Map visual tiers to stylized color profiles
  ...
  // Estimate components of leveling scores back from raw level for visual flavor
  // Score formula was roughly: (commits * 0.5) + (stars * 5) + (followers * 2) + (repos * 1) + (years * 10)
  // Let's print the actual stats. (Wait, our Socket player state didn't pack ALL raw stats to keep packet size small.
  // Instead, let's render a gorgeous card with estimated or fetched stats, and let's add visual stat bars!).
  // Wait, can we fetch the full stats of this user from our SQLite database on click? Yes! We can fetch `/api/leaderboard` or make a direct query. Or we can pass the player's level, score, title and estimate them, or simply fetch them if needed.
  // Actually, let's keep it simple: the player row in Socket has level, score, title, visual_tier. Since score is present, we can display their total DevScore and map the level bar progress:
  // Next Level formula: score required for level N is roughly: 5 * (N-1)^2
