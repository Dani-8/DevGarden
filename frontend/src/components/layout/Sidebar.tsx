...
export default function Sidebar({
    ...
    return (
        <>
            <aside
                className={`h-full z-30 flex flex-col bg-[#faf6eb] border-r-4 border-[#3a2f28] transition-all duration-300 select-none text-[#3a2f28] flex-shrink-0 ${collapsed ? 'w-[65px]' : 'w-[200px]'
                    }`}
            >
                <div className="p-3 border-b-2 border-[#3a2f28]/10 min-h-[52px]">
                    {!collapsed && (
                ...
                <div className="p-3 border-b-2 border-[#3a2f28]/10 flex flex-col gap-2.5">
                    <div className={`flex items-center gap-2.5 ${collapsed ? 'justify-center' : ''}`}>
                        <img
                            src={user.avatar_url}
                            alt={user.username}
                            className="w-8 h-8 rounded-full border-2 border-[#3a2f28] object-cover shadow-sm grayscale sepia hue-rotate-[70deg] saturate-[2.5] brightness-[0.9] contrast-[1.1] opacity-85 hover:filter-none hover:opacity-100 hover:scale-105 transition-all duration-300 cursor-pointer"
                            referrerPolicy="no-referrer"
                        />
                        {!collapsed && (
                            <div className="flex flex-col min-w-0 animate-fadeIn">
                                <span className="text-xs font-bold text-[#3a2f28] truncate font-sans">
                                    {user.username}
                                </span>
                                <div className="flex items-center gap-1 mt-0.5">
                                    ...
                                    <span className="text-[9px] text-[#514339]/80 italic capitalize font-serif truncate max-w-[100px]">
                                        {user.title || 'Gardener'}
                                    </span>
                                </div>
                            </div>
                    ...
                    {!collapsed && (
                        <div className="grid grid-cols-3 gap-0.5 bg-[#e3d8c1]/35 p-1.5 rounded-lg border border-[#3a2f28]/10 text-center font-mono text-[8px] text-[#3a2f28] animate-fadeIn">
                            <div className="flex flex-col">
                                <span className="font-extrabold text-[9px] text-amber-900">
                                    {user.commits}
                                </span>
                                <span className="text-[7px] opacity-75">Commits</span>
                            </div>
                            <div className="flex flex-col border-x border-[#3a2f28]/10">
                                <span className="font-extrabold text-[9px] text-amber-900">
                                    {user.stars}
                                </span>
                ...
                <nav className="flex-1 px-2.5 py-3 flex flex-col gap-1.5">
                    <button
                        ...
                        className={`w-full py-2 px-2.5 rounded-lg border-2 transition-all flex items-center gap-2 font-serif text-[11px] font-bold cursor-pointer select-none ${collapsed ? 'justify-center' : ''
                                ...
                                : 'bg-[#e3d8c1]/40 border-[#3a2f28]/15 hover:bg-[#e3d8c1]/75 text-[#3a2f28]'
                            }`}
                    ...
                    >
                        ...
                        {!collapsed && (
                            <div className="flex items-center justify-between flex-1">
                                ...
                                <span className="bg-black/10 px-1 py-0.2 rounded text-[8px] font-mono font-bold text-[#3a2f28]/70">[K]</span>
                            </div>
                        )}
                    </button>
                    ...
                    {collapsed ? (
                        <button
                            ...
                            className={`w-full py-2 px-2.5 rounded-lg border-2 transition-all flex items-center justify-center cursor-pointer select-none ${goldenWater
                                    ...
                                    : 'bg-[#e3d8c1]/40 border-[#3a2f28]/15 text-[#3a2f28] hover:bg-[#e3d8c1]/75'
                                }`}
                        ...
                        <div className={`mt-2 p-3 rounded-xl border-2 flex flex-col gap-2 transition-all relative overflow-hidden ${goldenWater
                                ? 'bg-gradient-to-br from-amber-500/15 to-yellow-600/5 border-amber-500/50 shadow-[0_2px_4px_rgba(58,47,40,0.05)]'
                                : 'bg-[#e3d8c1]/20 border-dashed border-[#3a2f28]/25'
                            }`}>
                            {goldenWater && (
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-1.5">
                                    <Star className={`w-4 h-4 flex-shrink-0 text-amber-600 ${goldenWater ? 'fill-current animate-spin' : 'animate-pulse'}`} style={{ animationDuration: goldenWater ? '12s' : '2s' }} />
                                    <span className="font-serif font-bold text-[11px] text-amber-900 tracking-wide uppercase">
                                        Golden Water
                                    </span>
                                </div>
                            </div>
                            ...
                            {goldenWater ? (
                                <>
                                    <p className="text-[9px] text-[#514339] leading-normal font-sans">
                                ...
                                <>
                                    <p className="text-[9px] text-[#514339]/80 leading-normal font-sans">
                    ...
                    {isNearLeaderboard && !collapsed && (
                        <div className="mx-1 bg-[#ffae34]/20 border border-[#ffae34]/40 rounded-lg p-1.5 flex items-center gap-1.5 animate-pulse">
                            <Sparkles className="w-3 text-amber-700 flex-shrink-0" />
                            <span className="text-[8px] font-mono text-amber-950 leading-tight">
                ...
                <div className="p-2.5 border-t-2 border-[#3a2f28]/10 flex flex-col gap-1.5">
                    <button
                        onClick={onLogout}
                        className={`w-full py-1.5 px-2.5 rounded-lg flex items-center gap-2 font-sans text-[11px] font-bold transition-all border border-transparent hover:border-red-600/30 hover:bg-red-500/5 text-[#514339]/80 hover:text-red-700 cursor-pointer ${collapsed ? 'justify-center' : ''
                            }`}
                    ...
                    <button
                        ...
                        className="w-full mt-0.5 py-2 rounded bg-[#e3d8c1]/45 hover:bg-[#e3d8c1]/75 text-[#3a2f28]/70 hover:text-[#3a2f28] flex items-center justify-center transition-colors cursor-pointer"
